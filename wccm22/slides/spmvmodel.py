...
def arithmetic_intensity_tensor(b,n,decompose=True):
    'n = p+1, b = block size'
    if decompose:
        ncontractions = 6
    else:
        ncontractions = 9
    fma_lib     = ncontractions*(3+2*b)*n**4 + (31+18*b)*n**3
...
def arithmetic_intensity_tensor_qstore(b,n,decompose=True):
    'n = p+1, b = block size'
    if decompose:
        ncontractions = 6
    else:
        ncontractions = 9
    fma_lib     = ncontractions*(2*b)*n**4
    fma_physics = (1 + 3**2) * b**2 * n**3    # v . K_00 . u + Dv : K_11 : Du
    fma_all     = fma_lib + fma_physics
    stored_mem  = (b*(b+1)/2 + 3*b*(3*b+1)/2) * n**3 # Symmetric matrix for K_00 and K_11
    memory_in   = b*n**3
    unique_in   = b*(n-1)**3
    memory_out  = b*n**3
    unique_out  = b*(n-1)**3
...
def arithmetic_intensity_nontensor_qstore(b,n):
    'n = p+1, b = block size'
    fma_lib = (3+1) * n**6 * b * 2
    fma_physics = (1 + 3**2) * b**2 * n**3
    fma_all = fma_lib + fma_physics
    stored_mem  = (b*(b+1)/2 + 3*b*(3*b+1)/2) * n**3 # Symmetric matrix for K_00 and K_11
    memory_in   = b*n**3
    unique_in   = b*(n-1)**3
...
def plot_intensity(opts):
    ...
    print('tensor_qstore:', arithmetic_intensity_tensor_qstore(1,3))
    print('nontensor_qstore:', arithmetic_intensity_nontensor_qstore(1,3))
    ...
    for i,b in enumerate(brange):
        ...
        tensorq = np.array([arithmetic_intensity_tensor_qstore(b,n) for n in nrange])
        nontensorq = np.array([arithmetic_intensity_nontensor_qstore(b,n) for n in nrange])
        ...
        ax1.semilogy(nrange-1, tensorq[:,1], '<', linestyle=styles[i], label='tensor-qstore $b=%d$'%b)
        ax2.semilogy(nrange-1, tensorq[:,0], '<', linestyle=styles[i], label='tensor-qstore $b=%d$'%b)
        ax1.semilogy(nrange-1, nontensorq[:,1], '>', linestyle=styles[i], label=f'nontensor-qstore $b={b}$')
        ax2.semilogy(nrange-1, nontensorq[:,0], '>', linestyle=styles[i], label='nontensor-qstore $b={b}$')
    ax1.set_xlabel('polynomial degree')
    ax2.set_xlabel('polynomial degree')
    ax2e.set_xlabel('polynomial degree')
    ax1.set_ylabel('bytes/dof')
    ax2.set_ylabel('flops/dof')
    idx = [0,1,2,3,4,5]
    legend = plt.figlegend(list(ax1.lines[i] for i in idx),
                           list(ax1.lines[i].get_label() for i in idx),
                           #(0.555,0.6), frameon=False)
                           loc=(0.67,0.12), frameon=True)
    plt.axes(ax2); plt.xticks(nrange-1); plt.ylim(25)
